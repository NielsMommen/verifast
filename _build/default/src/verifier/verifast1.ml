...
module type VERIFY_PROGRAM_ARGS = sig
  ...
  type symbol
  type termnode
  val ctxt: (typenode, symbol, termnode) Prover.Prover_api.context
  val options: options
...
(*     append_nodups predinstmap predinstmap0 (fun (p, is) -> p ^ "(" ^ String.concat ", " is ^ ")") l "predicate instance", *)
              ...
              match try_assoc path !headermap with
                ...
                let (headers', ds) =
                  ...
                  | Java ->
                    ...
                    let ds = Java_frontend.Bridge.parse_java_files (List.map (fun javaspec -> concat pathDir javaspec) javaspecs) [] 
          ...
          | Java -> begin
            ...
            match try_assoc rtpath !headermap with
              | None -> 
                ...
                let ds = Java_frontend.Bridge.parse_java_files (List.map (fun x -> concat rtdir x) javaspecs) [] reportRange
                ...
                let prelude_name, parse_header_file = 
                  ...
                  | Some Cxx -> 
                    "prelude_cxx.h", fun report_macro_call path report_range report_should_fail verbose include_paths define_macros enforce_annotations data_model -> 
                      let module Translator = Cxx_frontend.Ast_translator.Make(
                        struct 
                      ...
                      with
                      | Cxx_frontend.Annotation_parser.CxxAnnParseException (l, msg)
                      | Cxx_frontend.Ast_translator.CxxAstTranslException (l, msg) -> static_error l msg None
                      end
  ...
  let make_getter type_name tt csym subtype fnames f tp =
    ...
    let get = ctxt#mk_symbol id [typenode_of_type tt] (typenode_of_type tp) (Prover.Prover_api.Fixpoint (subtype, 0)) in
  ...
  let make_setter type_name tt csym subtype getters fnames f tp =
    ...
    let set = ctxt#mk_symbol id [tt1; tp1] tt1 (Prover.Prover_api.Fixpoint (subtype, 0)) in
    ...
    let rec iter (pn,ilist) imap pfm fpm ds =
        ...
        let rec citer j ctormap pfm ctors =
          ...
          | Ctor (lc, cn, name_and_type_exprs)::ctors ->
            ...
            if List.mem_assoc full_cn pfm || List.mem_assoc full_cn purefuncmap0 then
              ...
              let csym =
                mk_func_symbol full_cn (List.map provertype_of_type ts) ProverInductive (Prover.Prover_api.Ctor (CtorByOrdinal (subtype, j)))
              in
      ...
      | Func (l, Fixpoint, tparams, rto, g, ps, nonghost_callers_only, functype, contract, terminates, body_opt, _, _)::ds ->
        ...
        begin match body_opt with
          ...
          in
          let fsym = mk_func_symbol g (List.map (fun (p, t) -> provertype_of_type t) pmap) (provertype_of_type rt) (Prover.Prover_api.Fixpoint (subtype, index)) in
        ...
        | Some ([ReturnStmt (lr, Some e) as body], _) ->
          let fsym = mk_func_symbol g (List.map (fun (p, t) -> provertype_of_type t) pmap) (provertype_of_type rt) Prover.Prover_api.Uninterp in
    ...
    structmap1 |> flatmap (fun (sn, (l, body_opt, _, _, _)) ->
        ...
        *)
        ...
        | Some (_, fds, _) ->
          ...
          let subtype = InductiveSubtype.alloc () in
          let (csym, _) = mk_func_symbol cname (List.map (fun (x, t) -> provertype_of_type t) field_types) ProverInductive (Prover.Prover_api.Ctor (CtorByOrdinal (subtype, 0))) in
    ...
    let rec iter (pn,ilist) pcm pfm ds =
      match ds with
        ...
        let ps2 =
          ...
          iter ps1 [] ps2
        in
        let funcsym = mk_func_symbol p (List.map (fun (x, t) -> provertype_of_type t) ps1) ProverInductive Prover.Prover_api.Uninterp in
...
(* if the language is java, enable reasoning about instanceof *)
  ...
  let rec eval_core_cps0 eval_core ev state ass_term read_field env e cont =
    ...
    | WSwitchExpr (l, e, i, targs, cs, cdef_opt, tenv, tp) ->
      ...
      let symbol = ctxt#mk_symbol g (typenode_of_type tt :: List.map (fun (x, _) -> typenode_of_type (List.assoc x tenv)) env) (typenode_of_type tp) (Prover.Prover_api.Fixpoint (subtype, 0)) in
