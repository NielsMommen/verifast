...
let _ =
  let verify ?(emitter_callback = fun _ -> ()) (print_stats : bool) (options : options) (prover : string) (path : string) (emitHighlightedSourceFiles : bool) (dumpPerLineStmtExecCounts : bool) allowDeadCode json mergeOptionsFromSourceFile breakpoint focus targetPath =
    let exit l =
      Java_fe.Frontend_bridge.unload();
      exit l
    in
  ...
  let cla = cla @
            ...
            ; "-emit_sexpr",
              String begin fun str ->
                ...
                Vf_sexpr.Emitter.unsupported_exception := false
              end,
              "Emits the AST as an s-expression to the specified file."
            ; "-emit_sexpr_fail",
              String begin fun str ->
                outputSExpressions := Some str;
                Vf_sexpr.Emitter.unsupported_exception := true
              end,
            ...
            ; "-javac", Unit (fun _ -> (useJavaFrontend := true; Java_fe.Frontend_bridge.load ())), " "
        ...
        let emitter_callback (packages : package list) =
          match !outputSExpressions with
            | Some target_file ->
              ...
              Vf_sexpr.Emitter.emit target_file packages          
            | None             -> ()
        in
