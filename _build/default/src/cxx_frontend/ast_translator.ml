module Stubs = Cxx_stubs_ast.Make(Capnp.BytesMessage)
...
type 'a capnp_arr = (Cxx_stubs_ast.ro, 'a, R.array_t) Capnp.Array.t
...
module Make (Args: Fe_sig.CXX_TRANSLATOR_ARGS) : Fe_sig.Cxx_Ast_Translator = struct
  ...
  *)
  ...
  and transl_record_decl (loc: VF.loc) (record: R.Decl.Record.t): VF.decl list =
    ...
    let transl_mems (decls: R.Node.t list): Fe_sig.struct_member_decl list =
      let transl_mem decl =
        ...
        | R.Decl.Field f ->
          ...
          [Fe_sig.CxxFieldMem field]
        | _ ->
          transl_decl decl |> List.map (fun d -> Fe_sig.CxxDeclMem d)
      in
  ...
  *)
  ...
  let transl_tu (tu: R.TU.t): Fe_sig.header_type list * VF.decl list =
    let open R.TU in
  ...
  let parse_cxx_file (): Fe_sig.header_type list * VF.package list =
