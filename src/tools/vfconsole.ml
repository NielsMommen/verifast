    ...
    try
      ...
      let reportDirective directive l =
        ...
        | _ ->
          false
      in
      let callbacks = {Verifier.Verifast1.reportRange=range_callback; reportStmt; reportStmtExec; reportDirective; reportUseSite; reportExecutionForest} in
      let prover, options = 
  ...
  let cla = cla @
            ...
            ; "-emit_sexpr",
              String begin fun str ->
                ...
                Sexpr.Emitter.unsupported_exception := false
              end,
              "Emits the AST as an s-expression to the specified file."
            ; "-emit_sexpr_fail",
              String begin fun str ->
                outputSExpressions := Some str;
                Sexpr.Emitter.unsupported_exception := true
              end,
            ...
            ; "-javac", Unit (fun _ -> (useJavaFrontend := true; Java_frontend.Bridge.load ())), " "
        ...
        let emitter_callback (packages : package list) =
          begin match !dumpAST with
            | Some (noLocs, target_file) ->
              Ocaml_expr.Formatter.print_ocaml_expr_to_file noLocs target_file (Ocaml_expr.Of_ast.of_list Ocaml_expr.Of_ast.of_package packages)
            | None -> ()
          end;
          match !outputSExpressions with
            | Some target_file ->
              Printf.printf "Emitting s-expressions to %s\n" target_file;
              Sexpr.Emitter.emit target_file packages          
            | None             -> ()
        in
