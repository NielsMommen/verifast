...
let show_ide initialPath prover codeFont traceFont vfbindings layout javaFrontend enforceAnnotations verifyAndQuit =
  ...
  let leftBranchPixbuf = Branch_png.Branchleft_png.pixbuf () in
  let rightBranchPixbuf = Branch_png.Branchright_png.pixbuf () in
  let ctxts_lifo = ref None in
  let msg = ref None in
  let url = ref None in
  let appTitle = "VeriFast" ^ string_of_string_opt Verifier.Verifast.version ^ " IDE" in
  ...
  let showBannerDialog () =
    ...
    ignore $. GMisc.label ~xpad:2 ~ypad:2 ~line_wrap:true ~text:(Verifier.Verifast.banner ()) ~packing:vbox#pack ();
