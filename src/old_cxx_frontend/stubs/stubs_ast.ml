  ...
  module Reader = struct
    ...
    module Decl = struct
      ...
      let function_template_get_pipelined x =
        MessageWrapper.Untyped.struct_field x 0
      let has_class_template x =
        RA_.has_field x 0
      let class_template_get x =
        RA_.get_struct x 0
      let class_template_get_pipelined x =
        MessageWrapper.Untyped.struct_field x 0
      type unnamed_union_t =
        | UnionNotInitialized
        | Ann of string
      ...
      let get x =
        ...
        | 15 -> ClassTemplate (class_template_get x)
        | v -> Undefined v
      let of_message x = RA_.get_root_struct (RA_.Message.readonly x)
      let of_builder x = Some (RA_.StructStorage.readonly x)
    end
