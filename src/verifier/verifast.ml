    ...
    let (headers, ds)=
      match file_specs path with
        Java, _ ->
          ...
          let ds = Java_frontend.Bridge.parse_java_files javas context reportRange reportShouldFail options.option_verbose options.option_enforce_annotations options.option_use_java_frontend in
          (headers, ds)
      | CLang, None ->
...
let verify_program_core (* ?verify_program_core *)
    ...
    (ctxt: (typenode', symbol', termnode') Prover.Prover_api.context)
    (options : options)
    (program_path : string)
    (callbacks : callbacks)
...
class virtual prover_client =
  object
    method virtual run: 'typenode 'symbol 'termnode. ('typenode, 'symbol, 'termnode) Prover.Prover_api.context -> Stats.stats
  end
...
let verify_program (* ?verify_program *)
  ...
  lookup_prover prover
    (object
       method run: 'typenode 'symbol 'termnode. ('typenode, 'symbol, 'termnode) Prover.Prover_api.context -> Stats.stats =
